...
func GetSMTPClient(host, port, username, password string) (*smtp.Client, error) {
	...
	if err != nil {
		c.base.log.Debug().Msgf("smtp: DialTLS连接失败")
		client, err = smtp.DialStartTLS(host+":"+port, tlsConfig)
		if err != nil {
			c.base.log.Debug().Msgf("smtp: DialStartTLS连接失败")
	...
	if err := client.Hello("localhost"); err != nil {
		c.base.log.Debug().Msgf("smtp: SMTP服务器不可用: %v", err)
	...
	if err := client.Auth(auth); err != nil {
		c.base.log.Debug().Msgf("smtp: SMTP认证失败: %v", err)
